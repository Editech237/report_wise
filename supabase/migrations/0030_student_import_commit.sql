...
declare
  ...
  v_count integer := 0;
begin
  select * into v_batch from public.student_import_batches where id = p_batch;
  ...
  for item in select * from public.student_import_rows where batch_id = p_batch and status = 'APPROVED' order by row_number loop
    ...
    if not exists (select 1 from public.classes where id = v_class and school_id = v_batch.school_id and academic_year_id = p_year) then
...
exception when others then
  update public.student_import_batches set status = 'FAILED', error_message = sqlerrm, updated_at = now() where id = p_batch;
  raise;
end;
$$;
